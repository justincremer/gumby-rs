...
pub fn get_urls(handle: Handle) -> Vec<String> {
    ...
    get_elem_by_name(handle, "a", &mut anchor_tags);
...
fn get_elem_by_name(handle: Handle, element_name: &str, out: &mut Vec<NodeData>) {
    let node = handle;
    ...
    for n in node.children.borrow().iter() {
        get_elem_by_name(n.clone(), element_name, out);
    }
}
